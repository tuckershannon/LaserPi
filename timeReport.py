    ...
    for x in range(0,len(data)):
        ...
        if stuff != []:
            ...
            if 'A' == stuff[0]:
                ...
                while abs(int(startAngle) - int(endAngle))>10:
...
def main():
    ...
    messages = [datetime.now().strftime('%-I:%M')]
    for lol in range(0,len(messages)):
        startX = 0
        letterStart = 0.0
        startPosition = -(((0.2 / 2) *  ((float(len(list(messages[lol])))/1.0))))
            ...
            if letter == " ":
                ...
                for lineDraw in letter:
                    for x in range(0,2):
                        ...
                        laser(x)
                        nextX = float(lineDraw[x*2])  * (0.4 / 7) +  letterStart + startPosition
                        nextY = float(lineDraw[x*2+1])  * (0.4 / 7) #+ (0.30) * (len(messages)/2.0-lol)
                        if (float(lineDraw[x*2])  * (0.4 / 7)) > farXLetter:
                            farXLetter = (float(lineDraw[x*2])  * (0.4 / 7))
                        dx = currentX - nextX
                        dy = currentY - nextY
                        ...
                        for i in range(0,len(xStepArray)):
                            ...
                            if abs(yStepArray[i]-currentY) > currentDY:
                                if yStepArray[i]-currentY > 0:
                                    ...
                                    currentPhi = currentPhi - radPerStep
            laser(0)
            letterStart = letterStart + farXLetter + 0.1
